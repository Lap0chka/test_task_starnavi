...
class Post(models.Model):
    ...
    def clean(self) -> None:
        """
        Validates the post title and body for inappropriate language using `check_swearing`.
        If the title or body contains swear words, increments the `amount_block_comment`
        counter of the associated post and raises a `ValidationError`.
        """
        if check_swearing(self.body) or check_swearing(self.title):
...
class Comment(models.Model):
    ...
    def clean(self) -> None:
        """
        Validates the comment body for inappropriate language using `check_swearing`.
        If the body contains swear words, increments the `amount_block_comment`
        counter of the associated post and raises a `ValidationError`.
        """
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        ...
        super().save(*args, **kwargs)
        if self.post.auto_response_comment:
            time_response_minutes = self.post.time_response
            set_auto_response_parent.apply_async((self.id,),
